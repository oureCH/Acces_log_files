            ...
            with open(file_path) as f:
                ...
                for url, status_counts in data.items():
                    ...
                    for status, count in status_counts.items():
                        ...
                        url_data[url][status]['dates'].add(filename.split('-')[-1].split('.')[0])
    ...
    for url, status_counts in url_data.items():
        for status, count_and_dates in status_counts.items():
            summary_data.append({
                ...
                'date': ', '.join(sorted(count_and_dates['dates']))
            })
...
def display_summary_table(summary_data):
    ...
    print(f"{'Endpoints':<{max_url_length}} {'Status':<10} {'Calls':<5} {'Date':<20}")
    for data in sorted(summary_data, key=lambda x: (x['endpoints'], x['status'], -x['calls'], x['date'])):
        print(f"{data['endpoints']:<{max_url_length}} {data['status']:<10} {data['calls']:<5} {data['date']:<20}")
